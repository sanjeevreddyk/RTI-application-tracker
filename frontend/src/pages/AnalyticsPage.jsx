      ...
      <Grid2 container spacing={2}>
        <Grid2 size={{ xs: 12, md: 4 }}>
          <Paper sx={{ p: 2, background: 'linear-gradient(145deg, #ffffff 0%, #f4fbff 100%)' }}>
            ...
            <Typography variant="h4" color="primary.main" fontWeight={800}>{data?.averageResponseTime || 0}</Typography>
          </Paper>
        </Grid2>
        <Grid2 size={{ xs: 12, md: 4 }}>
          <Paper sx={{ p: 2, background: 'linear-gradient(145deg, #ffffff 0%, #fffaef 100%)' }}>
            ...
            <Typography variant="h4" color="secondary.main" fontWeight={800}>{data?.appealSuccessRate || 0}%</Typography>
          </Paper>
        </Grid2>
      </Grid2>
      ...
      <Grid2 container spacing={2}>
        <Grid2 size={{ xs: 12, md: 6 }}>
          <Paper sx={{ p: 2, height: 340, background: 'linear-gradient(180deg, #ffffff 0%, #f8fbfe 100%)' }}>
            ...
            <ResponsiveContainer width="100%" height="90%">
              <BarChart data={data?.departmentsDelay || []}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="department" hide={false} />
                <YAxis />
                <Tooltip />
        ...
        <Grid2 size={{ xs: 12, md: 6 }}>
          <Paper sx={{ p: 2, height: 340, background: 'linear-gradient(180deg, #ffffff 0%, #f8fbfe 100%)' }}>
            ...
            <ResponsiveContainer width="100%" height="90%">
              <BarChart data={data?.rtisFiledPerYear || []}>
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis dataKey="year" />
                <YAxis />
                <Tooltip />
